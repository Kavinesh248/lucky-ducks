...
class WithdrawController extends Controller
{
            ...
            if ($request->amount > 0) {
                ...
                if ($from->affilate_income >= $amount) {
                    ...
                    $finalamount = $amount - $fee;
                     ...
                     if($finalamount <= 0){
                         return response()->json(['status' => false, 'data' => [], 'error' => ["message" => 'You cannot check out with an amount less than the charge.']]);
                    }
    ...
    public function convertSubmit(Request $request)
    {
        ...
        $trans->txnid = Str::random(3).substr(time(), 6,8).Str::random(3);
    ...
    public function getReword(){
        $user = Auth::user();
        $datas = Transaction::where('details','Reward Point Convert')->where('user_id',$user->id)->orderby('id','desc')->get();
        $datas->toArray();
